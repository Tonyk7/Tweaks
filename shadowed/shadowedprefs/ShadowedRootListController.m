...
@interface UIApplication (Shadowed)
-(void)openURL:(id)arg1 options:(id)arg2 completionHandler:(id)arg3 ;
@end


@interface ShadowedTitleCell : PSTableCell <PreferencesTableCustomView> {
	UILabel *tweakTitle;
...
@implementation ShadowedTitleCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(id)reuseIdentifier specifier:(id)specifier {
	...
	if (self) {
		...
		[tweakSubtitle setText:@"Hide apps in switcher"];
...
- (instancetype)initWithSpecifier:(PSSpecifier *)specifier {
	return [self initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ShadowedTitleCell" specifier:specifier];
}
...
-(void)twitterButton2 {
	if ([application canOpenURL:[NSURL URLWithString:@"twitter://"]])
		[application openURL:[NSURL URLWithString:@"twitter://user?screen_name=IndieDevKB"] options:@{} completionHandler:nil];
	else	
		[application openURL:[NSURL URLWithString:@"https://twitter.com/IndieDevKB"] options:@{} completionHandler:nil];

}

-(void)githubButton {
	[application openURL:[NSURL URLWithString:@"https://github.com/Tonyk7/Tweaks/tree/master/shadowed"] options:@{} completionHandler:nil];
}
